...
def load_settings():
    ...
    if not path.exists():
        print("settings.json not found", path)
        sys.exit(1)
    with open(path) as f:
        return json.load(f)
...
def format_due_date(due_str):
    ...
    try:
        ...
        h = abs(hours)
        if h < 1:
            label = "<1h ago" if late else "in <1h"
        elif h < 24:
            label = f"{int(h)}h ago" if late else f"in {int(h)}h"
        else:
            label = f"{int(h/24)}d ago" if late else f"in {int(h/24)}d"
        return label, late
    except Exception:
...
def measure_card_content_height(draw, card, fonts, cfg):
    """Return the height of the card area that contains useful content."""
    inner_w = COLUMN_WIDTH - 2 * COLUMN_PADDING - 2 * CARD_PADDING
    h = CARD_PADDING
    h += len(wrap_text(draw, card.get("name", "Untitled"), fonts["bold"], inner_w)) * 13
    desc_lines = clamp_desc_rows(
...
def draw_checkbox(draw, x, y, checked):
    ...
    if checked:
        draw.line(
            [(x + 1, y + 4), (x + 3, y + 7), (x + 7, y + 1)], fill=(30, 30, 30), width=1
        )
...
def filter_columns(columns, cfg):
    result = []
    for col in columns:
        if cfg["skip_done"] and col.get("title", "").strip().lower() == "done":
            continue
        if cfg["skip_empty"] and not col.get("cards"):
            continue
        result.append(col)
...
def render_full(board, fonts, cfg):
    ...
    Returns:
        ...
        card_content_bounds -- list of (x_left, y_top, x_right, y_bottom) for useful card content
    """
    ...
    for col in columns:
        ...
        for card in col.get("cards", []):
            h = measure_card_height(draw, card, fonts, cfg)
            content_h = measure_card_content_height(draw, card, fonts, cfg)
            card_y_bounds.append((cy, cy + h))
            card_content_bounds.append(
                (
                    cx + COLUMN_PADDING,
                    cy,
                    cx + COLUMN_WIDTH - 2 * COLUMN_PADDING,
                    cy + content_h,
                )
            )
            cy += h + CARD_GAP
    ...
    return img, col_x_bounds, card_y_bounds, card_content_bounds
...
def stamp_label(img, page_num, total_pages, fonts):
    ...
    pad = 4
    draw.rectangle(
        [pad, pad, pad + lw + 6, pad + 15], fill=BG_BOARD, outline=(180, 180, 180)
    )
    draw.text((pad + 3, pad + 2), label, font=fonts["small"], fill=TEXT_TITLE)


def tile_has_content(x0, y0, tile_w, tile_h, card_content_bounds):
    """Return True if the tile overlaps any useful card content."""
    x1 = x0 + tile_w
    y1 = y0 + tile_h
    for left, top, right, bottom in card_content_bounds:
        if x0 < right and x1 > left and y0 < bottom and y1 > top:
            return True
    return False
...
def save_board(board, fonts, cfg, out_dir, board_index):
    ...
    full_img, col_x_bounds, card_y_bounds, card_content_bounds = render_full(
        board, fonts, cfg
    )
    ...
    for yi, y0 in enumerate(y_starts):
        for xi, x0 in enumerate(x_starts):
            ...
            if not tile_has_content(x0, y0, tile_w, tile_h, card_content_bounds):
            ...
            tile.save(path)
            print(
                f"Saved: {path}  ({tile.width}x{tile.height})  [{tile_num}/{total_tiles}]"
            )
